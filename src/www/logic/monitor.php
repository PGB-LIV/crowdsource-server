...
if ($state['phase'] == 1 && $state['state'] == 'READY') {
    ...
    if ($queuePosition > 0) {
        ...
        $progress = $adodb->GetAssoc(
            'SELECT `status`, COUNT(`precursor`) FROM `workunit1` WHERE `job` = ' . $jobId . ' GROUP BY `status`');

        echo 'Total work: ' . number_format(array_sum($progress)) . '<br />';
        echo 'Completed: ' . number_format($progress['COMPLETE']) . ' (' .
            number_format($progress['COMPLETE'] / array_sum($progress) * 100, 2) . '%)<br />';
        echo 'Processing: ' . number_format($progress['ASSIGNED']) . ' (' .
            number_format($progress['ASSIGNED'] / array_sum($progress) * 100, 2) . '%)<br />';
        echo 'Awaiting: ' . number_format($progress['UNASSIGNED']) . ' (' .
            number_format($progress['UNASSIGNED'] / array_sum($progress) * 100, 2) . '%)<br />';

        $stat = $adodb->GetRow(
            'SELECT UNIX_TIMESTAMP(MIN(`completed_at`)) AS `start`, UNIX_TIMESTAMP(MAX(`completed_at`)) AS `finish` FROM `workunit1` WHERE `job` = ' .
            $jobId);
...
if ($state['phase'] == 1 && $state['state'] == 'COMPLETE') {
    ...
    echo '<p>Analyse results <a href="http://pgb.liv.ac.uk/shiny/ash/?data=http://pgb.liv.ac.uk/~andrew/crowdsource-server/src/public_html/?page=results&job=' .
        $jobId . '">here</a></p>';
}
